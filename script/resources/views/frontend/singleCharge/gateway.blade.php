                            ...
                            <div class="mt-4">
                                <table class="table table-bordered mb-4">
                                    ...
                                    <tr>
                                        <th>{{ __("Amount") }}</th>
                                        <td>
                                            @if($singleCharge->currency_id !== $gateway->currency_id)
                                                {{ currency_format($amount, currency: $singleCharge->currency) }}  =
                                            @endif
                                            @if(Auth::check())
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                $amountForGateway = convert_money_direct($amount, $singleCharge->currency, $gateway->currency);
                                            @endphp
                                ...
                                <form action="{{ route('frontend.single-charge.payment', [$singleCharge->uuid, $gateway->id]) }}" method="post" role="form" class="form-light form">
                                    @csrf
                                    <input type="hidden" value="single_charge" name="payment_type">
                                    @unless(Auth::check())
